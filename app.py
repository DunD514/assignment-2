...
.price{font-weight:bold;color:#22c55e;font-size:18px;}
</style>
</head>
<body>
...
<td>
<button class="btn btn-success btn-sm" onclick="bid('{{p}}')">Bid</button>
</td>
</tr>
{% endfor %}
</tbody>
...
function bid(player){
    ...
    if(price){
        socket.emit("place_bid",{player:player, bid:price});
    }
}
...
socket.on("price_update",function(data){
    ...
    setTimeout(()=>{priceCell.style.transform="scale(1)";},300);
    statusCell.innerHTML = "🔥 New Bid";
    setTimeout(()=>{statusCell.innerHTML = "Waiting";},2000);
});
